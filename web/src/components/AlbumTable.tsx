...
export default function AlbumTable({
  ...
  const columns: ColumnsType<DiscListItem> = [
    {
      title: "封面",
      dataIndex: "cover",
      width: 72,
      render: (cover: string, album) =>
        cover ? (
          <Image alt={`${album.title} cover`} className="cover-thumb" preview={false} src={cover} />
        ) : (
    ...
    },
    {
      ...
      render: (_, album) => (
        <Space>
          <Button icon={<EyeOutlined />} onClick={() => onShow(album.id)}>
            详情
          </Button>
          <Button disabled={syncDisabled} icon={<SyncOutlined />} onClick={() => onSync(album.id)}>
            同步
          </Button>
        </Space>
      ),
