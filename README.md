...
`[schedule]` 仅在 `--api-server` / Web GUI 模式下生效。开启后，服务会按 cron 表达式自动触发全量同步，并复用现有同步任务锁：如果手动同步或其他同步任务正在运行，本次自动同步会跳过。

Cron 表达式使用 `cron` crate 的 7 段格式：
...
[schedule]
enabled = true
cron = "0 0 3 * * * *" # 每天 03:00 同步
```

也可以在 Web UI 的“设置”页直接启用/关闭并修改表达式。
